...
export default defineComponent({
    ...
    setup(props, context) {
        ...
        const drawPic = () => {
            ...
            ctx.fillRect(0, 6, contentWidth.value, contentHeight.value)
            // 绘制文字
        ...
        const drawLine = (ctx: CanvasRenderingContext2D) => {
            for (let i = 0; i < 4; i++) {
                ...
                ctx.beginPath()
                ctx.moveTo(randomNum(0, contentWidth.value), randomNum(0+6, contentHeight.value))
                ctx.lineTo(randomNum(0, contentWidth.value), randomNum(0+6, contentHeight.value))
                ctx.stroke()
            }
        }
        // 绘制干扰点
        const drawDot = (ctx: CanvasRenderingContext2D) => {
            for (let i = 0; i < 60; i++) {
                ...
                ctx.beginPath()
                ctx.arc(randomNum(0, contentWidth.value), randomNum(0+6, contentHeight.value), 1, 0, 2 * Math
                    .PI)
                ctx.fill()
            }
        }
